...
public class HiloRangos implements Runnable {
    ...
    //esta repitiendo y asi el programa esta preparado para encontrar un
    //bucle diferente al 4,2,1 ademas de salir del while que
    //hace las operacipones cuando llega a ese u otro bucle
    ...
    public void run() {
        ...
        for (BigInteger i = inicioRango; i.compareTo(finRango)==-1 || i.compareTo(finRango)==0; i=i.add(BigInteger.ONE)) {
            ...
            //Mientras el value del numero que se va a calcular es nulo
            //se hacen los calculos, cuando el value no es nulo significa
            //que se ha entrado en un bucle, el 4,2,1 u otro distinto
            while (repetido.get(resultado) == null) {
                numActual=resultado;
                //Compruebo desde el hashmap de datos si
                //este numero ya esta calculado o no
                if (datos.calculados(numActual)==false) {
                    ...
                    //Dado que esta secuencia ya ha sido explorada, ya se ha
                    //comprobado si es el bucle 4,2,1 o es un nuevo bucle la
                    //primera vez que se calculo la secuencia,
                    //asi que dejo la variable a false 
                ...
                //Comparo el numero alcanzado para ver si es el maximo numero alcanzado
                if (resultado.compareTo(datos.getNumMax())==1){
                    datos.setNumMax(resultado);
                }    
            }
            //Si la variable esta en true, significa que esa secuencia no
            //se ha comprobado si desemboca en un nuevo bucle
            ...
            //Manda el contador de la secuencia junto con la semilla que provoca esa secuencia
    ...
    private void comprobarPosibleNuevoBucle(BigInteger numRepetido, BigInteger semilla) {
        //Si el numero que se repite no es el 4 o el 2 o el 1 y no es el 0, para
